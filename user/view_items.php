...
$base_url_profile = 'profile_picture/';

$base_url_student_reports = ''; // The browser path is 'user/inserted_report/filename' which is handled by the empty path + DB path
...
<body>
    ...
    <div class="pt-20 pb-10 bg-gray-50 min-h-screen">
        <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

            <header class="text-center py-8 mb-6 bg-white rounded-xl shadow-xl border-t-8 border-blue-600" data-aos="fade-down">
                <h1 class="text-4xl font-extrabold text-gray-800">
                    <i class="fas fa-search-location text-blue-600 mr-3"></i> Lost & Found Feed
                </h1>
                ...
                <div class="mt-6 p-4 bg-yellow-50 border-l-4 border-yellow-500 text-yellow-800 rounded-lg shadow-md mx-auto max-w-lg font-semibold text-base">
                    <i class="fas fa-exclamation-triangle mr-2"></i>
                    <strong class="text-yellow-900">Found your lost item?</strong> Visit the <b>Lost and Found Office</b> to claim it by providing a detailed description (which is not visible here).
                </div>
            </header>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <aside class="col-span-1 space-y-8">
                    <div class="bg-white p-6 rounded-xl shadow-lg border-t-4 border-red-500 sticky top-20" data-aos="fade-right">
    ...
    <div id="itemModal" class="hidden fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 p-4">
        <div class="bg-white rounded-xl shadow-2xl max-w-xl w-full max-h-[90vh] overflow-y-auto transform scale-90 opacity-0 transition-all duration-300">
            <div class="p-6">
                ...
                <img id="modalImage" src="" alt="Item Image" class="w-full max-h-72 object-contain rounded-lg mb-4 bg-gray-50 border p-2">
    ...
    <script>
        ...
        const BASE_URL_STUDENT_REPORTS = '<?= $base_url_student_reports ?>';
        const BASE_URL_ADMIN_REPORTS = '<?= $base_url_admin_reports ?>';


        // Helper function for status badge classes (unchanged)
        ...
        function getProfilePicture(item) {
            // DB stores only the filename. BASE_URL_PROFILE is 'profile_picture/'
            ...
            if (profileImagePath) {
                // Final Path Example: profile_picture/1760368071_....jpg
        ...
        /**
         * FIX: Now we simply concatenate the PHP Base URL and the DB Path (which already includes 'inserted_report/').
         */
        function getItemImagePath(item) {
            if (!item.image_path) return null; // image_path contains 'inserted_report/filename.jpg'

            if (item.reporter_type === 'Student') {
                // BASE_URL_STUDENT_REPORTS is '' (empty)
                // Final Path Example: inserted_report/1760553778_...png
                return `${BASE_URL_STUDENT_REPORTS}${item.image_path}`;
            } else if (item.reporter_type === 'Admin') {
                // BASE_URL_ADMIN_REPORTS is '../admin/'
                // Final Path Example: ../admin/inserted_report/1760611928_...jpg
        ...
        // --- Rendering Function (unchanged logic) ---
        function renderItems(data) {
            ...
            data.forEach(item => {
                ...
                const imageHtml = itemImagePath ?
                    `<img src="${itemImagePath}" alt="${item.item_name}" class="item-image w-full h-auto max-h-96 object-cover bg-gray-50 rounded-lg border-b shadow-sm cursor-pointer" onclick="openModal(${item.id})">` :
